...
command(Socket, <<78,73,67,75>>, NewNick, User) -> % NICK
    ...
    NewNick;
command(Socket, <<85,83,69,82>>, Message, User) -> % USER
...
command(Socket, <<80,73,78,71>>, Message, User) -> % PING send PONG
    ...
    User;
command(Socket, <<>>, Message, User) -> % QUIT
    User;
command(_, _, _, User) ->
    User.
